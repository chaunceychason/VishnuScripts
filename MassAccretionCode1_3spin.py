...
def plotMAtree3( scale1, mass1, spinarray1, half_formationscale, plotfunc_count):
	print("Entered Plot Function")
	plot_title="Mass Accretion History Tree: Spin"   #Can code the number in with treemax
	x_axis="scale time"
	y_axis="total spin"
	figure_name=os.path.expanduser('~/Jan20TOTALSpinAccretionfigure' +'.png')
	#Choose which type of plot you would like: Commented out.
	
	#plt.hist2d(scale1, mass1, (100, 100), cmap=plt.cm.jet)

	plt.plot(scale1, spinarray1, linestyle="-", marker="", markersize=3)
	#plt.plot(scale1, mass1, linestyle="", marker="o", markersize=3)
	#plt.plot(scale1, Vmaxarray, linestyle="-", marker="o")
	#plt.scatter(scale1, mass1, label="first tree")
	...
	clearmass  = []
	clearscale = []
	clearVmax  = []      #name doesnt really matter as long as it matches return statement. 
	plotfunc_count += 1
	return clearmass, clearscale, clearVmax, plotfunc_count

def plotMAtreeFINALhist( scale1, mass1, Vmaxarray, SpinarrayTOT,plotfunc_count):
	print("Entered Plot Function")
	plot_title="Mass Accretion Histogram: Vmax v. Spin"   #Can code the number in with treemax
	x_axis="Vmax"
	y_axis="Spin"
	figure_name=os.path.expanduser('~/Jan20HISTMassAccretionfigureSPIN' +'.png')
	#Choose which type of plot you would like: Commented out.
	...
	plt.hist2d(Vmaxarray, SpinarrayTOT, (100, 500), cmap=plt.cm.jet, norm=matplotlib.colors.LogNorm() )
					...
					if line.startswith("1.00000"):
						...
						if totaltrees == 0:
							...
							if goodmassvalue ==1:
								...
								totaltrees += 1	
								print("Total Trees Count %d " % totaltrees)
								#PLOT THE MASS ACCRETION HISTORIES. ALL ON TOP OF EACH OTHER FOR: MAtree2()
								#comment:  Plots the mass accretion histories, commented out for spin histories. 
								#mass1, scale1, Vmaxarray1, plotfunc_count  =  plotMAtree2(scale1, mass1, Vmaxarray1, half_formationscale, plotfunc_count)
								mass1, scale1, spinarray1, plotfunc_count  =  plotMAtree3(scale1, mass1, spinarray1, half_formationscale, plotfunc_count)
...
figure_name=os.path.expanduser('~/Jan19MAhistorytreeSPIN' +'.png')
plt.savefig(figure_name)
print("Saving plot: %s" % figure_name)
plt.clf()
...
plotMAtreeFINALhist(scaleTOT, massTOT, VmaxarrayTOT, SpinarrayTOT, plotfunc_count)
