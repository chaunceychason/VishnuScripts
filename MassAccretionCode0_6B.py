...
"""
use_specified_tree = 1
specified_tree = [9461473760]
			...
			with open(datafilename) as fp:
				for line in fp:
					...
					if line.startswith("1.00000"):   #Thus meaning a new halo at scale 1.0
						...
						FIND SPECIFIED TREE
						"""
						if use_specified_tree == 1:
							specified_tree_value = specified_tree[0]
							if  treeID != specified_tree_value:
								goodmassvalue = 0
								continue
							else:
								goodmassvalue = 1
								print("FOUND SPECIFIED TREE!")
...
figure_name=os.path.expanduser('~/Jan14MAhistorytree' +'.png')
plt.savefig(figure_name)
print("Saving plot: %s" % figure_name)
plt.clf()
